...
VALUES ('coach_sees_score', 'false'::jsonb, 'Mostrar el número de puntaje al coach en el modo Presentar y en el link público')
...
AS $function$
  SELECT jsonb_build_object(
    'config', jsonb_build_object(
      ...
      'coach_sees_score', COALESCE((SELECT value #>> '{}' FROM public.app_config WHERE key = 'coach_sees_score')::boolean, false)
    ),
    ...
    'previous', (
      SELECT jsonb_build_object(
        ...
        'result_phrase', pm.result_phrase,
    ...
    'recent_scores', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', x.class_date, 'score', x.final_score, 'phrase', x.result_phrase) ORDER BY x.created_at)
      FROM (
        SELECT lm.class_date, lm.final_score, lm.result_phrase, lm.created_at
