ALTER TABLE public.monitoring_answers
  ADD COLUMN evidence_time integer NULL;

ALTER TABLE public.monitoring_answers
  ADD CONSTRAINT monitoring_answers_evidence_time_nonnegative
  CHECK (evidence_time IS NULL OR evidence_time >= 0);

ALTER TABLE public.monitorings
  ADD COLUMN recording_start_time time NULL,
  ADD COLUMN coach_summary text NULL,
  ADD COLUMN coach_commitment text NULL,
  ADD COLUMN coach_responded_at timestamptz NULL,
  ADD COLUMN coach_counter text NULL,
  ADD COLUMN previous_commitment_status text NULL,
  ADD COLUMN class_timeline jsonb NULL;

ALTER TABLE public.monitorings
  ADD CONSTRAINT monitorings_previous_commitment_status_valid
  CHECK (previous_commitment_status IS NULL OR previous_commitment_status IN ('cumplido', 'parcial', 'no_cumplido')),
  ADD CONSTRAINT monitorings_coach_summary_length
  CHECK (coach_summary IS NULL OR char_length(coach_summary) <= 2000),
  ADD CONSTRAINT monitorings_coach_commitment_length
  CHECK (coach_commitment IS NULL OR char_length(coach_commitment) <= 2000),
  ADD CONSTRAINT monitorings_coach_counter_length
  CHECK (coach_counter IS NULL OR char_length(coach_counter) <= 2000);

CREATE OR REPLACE FUNCTION public.submit_coach_response(
  _token uuid,
  _summary text,
  _commitment text,
  _counter text DEFAULT NULL
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF _token IS NULL
     OR char_length(trim(COALESCE(_summary, ''))) = 0
     OR char_length(trim(COALESCE(_commitment, ''))) = 0
     OR char_length(trim(COALESCE(_summary, ''))) > 2000
     OR char_length(trim(COALESCE(_commitment, ''))) > 2000
     OR char_length(trim(COALESCE(_counter, ''))) > 2000 THEN
    RETURN false;
  END IF;

  UPDATE public.monitorings
  SET coach_summary = trim(_summary),
      coach_commitment = trim(_commitment),
      coach_counter = NULLIF(trim(COALESCE(_counter, '')), ''),
...
REVOKE ALL ON FUNCTION public.submit_coach_response(uuid, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_coach_response(uuid, text, text, text) TO anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.get_report_by_token(uuid) FROM PUBLIC, anon, authenticated;
