...
export async function fetchInternalReport(id: string): Promise<ReportData | null> {
  ...
  const [{ data: coach }, { data: template }, { data: profile }, { data: answers }, { data: students }, { data: previous }, { data: recent }, { data: configRows }] = await Promise.all([
    ...
    supabase.from("monitorings").select("id, class_date, final_score, result_phrase, transcript_metrics, aois, main_aoi, coach_commitment").eq("coach_id", m.coach_id ?? "").eq("status", "enviado").neq("id", id).lt("created_at", m.created_at).order("created_at", { ascending: false }).limit(1).maybeSingle(),
    supabase.from("monitorings").select("class_date, final_score, result_phrase, created_at").eq("coach_id", m.coach_id ?? "").eq("status", "enviado").not("final_score", "is", null).lte("created_at", m.created_at).order("created_at", { ascending: false }).limit(6),
    supabase.from("app_config").select("key,value").in("key", ["talk_time_green", "talk_time_yellow", "student_min_pct", "af_min_students", "coach_response_days", "coach_sees_score"]),
  ]);
  ...
  const coachSeesScore = String((configRows ?? []).find((row) => row.key === "coach_sees_score")?.value ?? "false") === "true";
  return {
    config: { talk_time_green: config["talk_time_green"] ?? 70, talk_time_yellow: config["talk_time_yellow"] ?? 55, student_min_pct: config["student_min_pct"] ?? 8, af_min_students: config["af_min_students"] ?? 3, coach_response_days: config["coach_response_days"] ?? 3, coach_sees_score: coachSeesScore },
    ...
    students: students ?? [],
    previous: previous ? { id: previous.id, date: previous.class_date, final_score: previous.final_score, result_phrase: previous.result_phrase, students_pct: prevMetrics?.students_pct ?? null, aois: previous.aois, main_aoi: previous.main_aoi, coach_commitment: previous.coach_commitment } : null,
    recent_scores: (recent ?? []).reverse().map((r) => ({ date: r.class_date, score: Number(r.final_score), phrase: r.result_phrase })),
  };
}