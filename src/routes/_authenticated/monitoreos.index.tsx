...
function MonitoringsPage() {
  ...
  const query = useQuery({ queryKey: ["monitorings", month], queryFn: async () => {
    ...
    let request = supabase.from("monitorings").select("id,coach_id,coordinator_id,template_id,class_date,qa_date,status,final_score,result_phrase,coach_responded_at,created_at,updated_at,coach:coaches(full_name),template:templates(name,code)").order("class_date", { ascending: false }).order("created_at", { ascending: false });
