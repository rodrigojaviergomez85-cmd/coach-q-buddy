...
import { monthlyAlerts, type MonthlyAlert } from "@/lib/follow-up";
...
function MonthlyPage() {
  ...
  const query = useQuery({ queryKey: ["monthly-dashboard", month], queryFn: async () => {
    ...
    const [coaches, monitorings, profiles, config] = await Promise.all([
      ...
      supabase.from("monitorings").select("id,coach_id,coordinator_id,class_date,qa_date,final_score,aois,main_aoi,coach_responded_at,transcript_metrics,penalty_applied,created_at,template:templates(name)").eq("status", "enviado").gte("qa_date", range.from).lt("qa_date", range.to).order("created_at", { ascending: true }),
      supabase.from("profiles").select("id,full_name,email").order("full_name"),
      supabase.from("app_config").select("key,value").in("key", ["monthly_target", "coach_response_days", "talk_time_yellow"]),
    ]);
